...
for t=tf-1:-1:dt
   V(:,:,t) = ((-S(:,:,t+1)*B*inv(R)*B' + 2*N*inv(R)*B' + A')*V(:,:,t+1) - S(:,:,t+1)*L*zr_dot)*(-dt) + V(:,:,t+1); % + C'*Q*ref;
end

%% X
X = zeros(4,tf/dt);
for t=1:tf/dt
    X(:,t+1) = ((A + 2*B*inv(R)*N' + B*inv(R)*B'*S(:,:,t))*X(:,t) - B*inv(R)*B'*V(:,:,t) + L*zr_dot)*dt + X(:,t);
end
